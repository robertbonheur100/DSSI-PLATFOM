        ...
        for inv in investments:
            ...
            if last_paid:
                ...
                if last_paid_dt > cutoff:
                    continue
            # Itilize to mansyèl PWÒP envestisman sa a (anrejistre lè l te
            # aktive), sinon defo aktyèl la (Config.MONTHLY_PROFIT_RATE).
            # Konsa yon plan ki te aktive sou 7% kontinye touche 7% jiskaske
            # li fini/reactive; yon plan aktive apre chanjman an touche 15%.
            rate    = inv.get('monthly_rate', Config.MONTHLY_PROFIT_RATE)
            profit  = round(inv['amount'] * rate, 2)
            user_id = inv['user_id']
            ...
            db.table('transactions').insert({
                ...
                'type':        'monthly_profit',
                'amount':      profit,
                'description': f"{rate*100:.0f}% mansyèl sou {inv['amount']:,} HTG ({inv.get('plan_name','Plan')})",
                'status':      'completed',
